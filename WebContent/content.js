...
function process() {
    ...
    if (document.body.firstChild.tagName=='PRE') {
        ...
        if (pre && pre.textContent){
            ...
            if(options['from_contextmenu']){
                if(pathname.substr(-4) !== ".css"
                    && pathname.substr(-3) !== ".js"){
                    if(document.location.href.indexOf('.js') !== -1){
                        pathname = '#fake.js';
                    }else{
                        try{
                            new Function(pre.textContent);
                            pathname = '#fake.js';
                        }catch(e){
                            pathname = '#fake.css';
                        }
            ...
            injectViewer(null, pre.textContent, pathname);
        }
    } else if (document.body.childNodes[0]
        && document.body.childNodes[0].className != "webkit-line-gutter-backdrop") {
        xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4){
                injectViewer(xhr.getResponseHeader("Content-Type"),
                    xhr.responseText,
